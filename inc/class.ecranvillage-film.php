...
class Film {
	...
	public static function save_meta( $post_id, $post ) {
		...
		if ( 'film' !== $post->post_type ||
			! isset( $_POST['film_meta_nonce'] ) ||
			! \wp_verify_nonce( \sanitize_text_field( \wp_unslash( $_POST['film_meta_nonce'] ) ), 'save_film_meta-' . $post_id ) ||
			! \current_user_can( 'edit_post', $post_id ) ) {
			return;
		}

		$film_post_meta['trailer'] = isset( $_POST['trailer'] ) ? \esc_url_raw( trim( \sanitize_text_field( \wp_unslash( $_POST['trailer'] ) ) ) ) : '';
		$film_post_meta['duree']   = isset( $_POST['duree'] ) ? \sanitize_text_field( \wp_unslash( $_POST['duree'] ) ) : '';
		$film_post_meta['info']    = isset( $_POST['info'] ) ? \sanitize_text_field( \wp_unslash( $_POST['info'] ) ) : '';
		$film_post_meta['info2']   = isset( $_POST['info2'] ) ? \sanitize_text_field( \wp_unslash( $_POST['info2'] ) ) : '';
		$film_post_meta['info3']   = isset( $_POST['info3'] ) ? \sanitize_text_field( \wp_unslash( $_POST['info3'] ) ) : '';
		...
		if ( empty( $film_post_meta['film_id'] ) ) {
			$id = \EcranVillage\API::get_film_id( $post->post_title );
			if ( ! \is_wp_error( $id ) ) {
		...
		if ( isset( $_POST['allocine'] ) ) {
			$alocine                    = \sanitize_text_field( \wp_unslash( $_POST['allocine'] ) );
			$film_post_meta['allocine'] = \ctype_digit( $alocine ) ? 'http://www.allocine.fr/film/fichefilm_gen_cfilm=' . $alocine . '.html' : \esc_url_raw( $alocine );
		} else {
		...
		// tmdb URL.
		if ( isset( $_POST['tmdb'] ) ) {
			$tmdb                   = \sanitize_text_field( \wp_unslash( $_POST['tmdb'] ) );
			$film_post_meta['tmdb'] = \ctype_digit( $tmdb ) ? 'https://www.themoviedb.org/movie/' . $tmdb : \esc_url_raw( $tmdb );
		} else {
			$film_post_meta['tmdb'] = '';
		}
		...
		if ( isset( $_POST['imdb'] ) ) {
			...
			$film_post_meta['imdb'] = ( 0 === \strpos( $imdb, 'tt' ) || is_numeric( $imdb ) ) ? 'https://www.imdb.com/title/' . $imdb . '/' : \esc_url_raw( $imdb );
		} else {
			$film_post_meta['imdb'] = '';
		}
